...
def test_get_token_success(monkeypatch):
    ...
    monkeypatch.setattr(sf.msal, "ConfidentialClientApplication", lambda *a, **k: DummyApp())
...
def test_get_latest_file_info_success(mock_list_files, mock_drive_id, mock_site_id, mock_get_token):
    file = {
        "name": "file.xlsx",
...
def test_get_latest_file_info_none(mock_list_files, mock_drive_id, mock_site_id, mock_get_token):
    info = sf.get_latest_file_info()
    assert info is None
...
@patch("utils.sharepoint_fetch.get_latest_file_info", return_value={"download_url": None})
