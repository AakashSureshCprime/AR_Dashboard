...
def empty_df():
    return pd.DataFrame(
        columns=[
            "Projection", "Total in USD", "Reference", "Remarks",
            "Customer Name", "New Org Name", "Allocation", "Entities",
            "AR Status", "AR Comments",
        ]
    )
...
class TestGetAllProjections:
    ...
    def test_excludes_empty_strings(self):
        df = pd.DataFrame({"Projection": ["Week1", "", "  ", "Week2"], "Total in USD": [1, 2, 3, 4]})
    ...
    def test_excludes_null_values(self):
        df = pd.DataFrame({"Projection": ["Week1", None, "Week2"], "Total in USD": [1, 2, 3]})
        ctrl = _make_controller(df)
        projections = ctrl._get_all_projections()
...
class TestSplitInflowDispute:
    ...
    def test_all_disputes_gives_empty_inflow_list(self):
        df = pd.DataFrame({"Projection": ["Dispute - A", "Dispute - B"], "Total in USD": [1, 2]})
        ctrl = _make_controller(df)
        inflow, dispute = ctrl._split_inflow_dispute()
        assert inflow == []
...
class TestSortKey:
    def test_month_ordering_jan_before_feb(self):
        assert ProjectionController._sort_key("Jan 1st week") < \
               ProjectionController._sort_key("Feb 1st week")

    def test_month_ordering_feb_before_mar(self):
        assert ProjectionController._sort_key("Feb 1st week") < \
    ...
    def test_week_rank_current_before_1st(self):
        assert ProjectionController._sort_key("Feb Current week") < \
               ProjectionController._sort_key("Feb 1st week")
    ...
    def test_next_month_sorts_after_explicit_months(self):
        assert ProjectionController._sort_key("Dec Last week") < \
...
class TestGetDueWiseOutstanding:
    def test_has_required_columns(self, controller):
        ...
        for col in ("Remarks", "Total Outstanding (USD)", "Invoice Count", "% of Total"):
...
class TestGetDueWiseDetail:
    ...
    def test_case_insensitive_matching(self, controller):
        assert len(controller.get_due_wise_detail("current due")) == \
               len(controller.get_due_wise_detail("CURRENT DUE"))
...
class TestGetAllocationRemarkDetail:
    ...
    def test_case_insensitive(self, controller):
        assert len(controller.get_allocation_remark_detail("nithya", "current due")) == \
...
class TestGetEntitiesRemarkDetail:
    ...
    def test_case_insensitive(self, controller):
        assert len(controller.get_entities_remark_detail("ust india", "current due")) == \
...
class TestGetARStatusRemarkDetail:
    ...
    def test_case_insensitive(self, controller):
        assert len(controller.get_ar_status_remark_detail("in progress", "current due")) == \
...
class TestEdgeCases:
    ...
    def test_large_dataset_weekly_summary_completes(self):
        ...
        n = 10_000
        df = pd.DataFrame({
            "Projection": rng.choice(["Feb 1st week", "Feb 2nd week", "Dispute - A"], n),
